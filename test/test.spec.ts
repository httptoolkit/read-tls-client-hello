...
describe("Read-TLS-Client-Hello", () => {
    ...
    it("can calculate JA4 fingerprint", async () => {
        server = makeDestroyable(new net.Server());
        ...
        const incomingSocket = await incomingSocketPromise;
        const helloData = await readTlsClientHello(incomingSocket);
        const ja4 = calculateJa4FromHelloData(helloData);

        // Verify format matches t{version}d{cipher_count}{ext_count}h2_{cipher_hash}_{ext_hash}
        expect(ja4).to.match(/^t(12|13)d\d+\d+(h2|00)_[a-f0-9]{12}_[a-f0-9]{12}$/);

        // Compare with echo.ramaproxy.org
        const response = await new Promise<EchoResponse>((resolve, reject) => {
            const req = https.get('https://echo.ramaproxy.org/');
            req.on('response', async (resp) => {
                if (resp.statusCode !== 200) reject(new Error(`Unexpected ${resp.statusCode}`));
                const rawData = await streamToBuffer(resp);
        ...
        expect(ja4).to.equal(response.tls.ja4.hash);
    });
